...
for (e in 1:NN) {
  ...
  # M_wh = solve(P_Z - t(WZ) %*% solve(W_qta) %*%  WZ) %*% (t(2 * Z_a) - t(WZ) %*% solve(W_qta) %*% t(2 * W_a))
  # N_wh = solve(W_qta  - WZ %*% solve(P_Z) %*% t(WZ)) %*% (t(2 * W_a) - WZ %*% solve(P_Z) %*% t(2 * Z_a))
  # eta_initial = M_wh %*% y
  # beta_initial = N_wh %*% y
  # 
  # beta_initial = solve(V_ZW + 2 * lambda_star * t(A) %*% A, U_ZW %*% y)
  # eta_initial = solve(P_Z, t(2 * Z_a) %*% y - t(WZ) %*% beta_initial)
  # 
  # Q_Z_true = (diag(n) - Z %*% solve(t(Z) %*% Z) %*% t(Z))
  # beta_initial_true = solve(t(W) %*% Q_Z_true %*% W + lambda_star * t(A) %*% A, t(W) %*% Q_Z_true %*% y)
  # eta_initial_true  = solve(t(Z) %*% Z, t(Z) %*% (y - W %*% beta_initial_true))         # q by 1
  # -------------------------------------------------------------------------
  ...
  # 
  # results <- parLapply(cl, lambda_list, Iteration_Ma_woe, W, Z, y, beta_initial_true, eta_initial_true, 3, 1, 2)
  # for (i in 1:k) {
  ...
  # index1 = seq(from = 1, to = n*p-1, by = p)
  # beta_hat1 = beta_hat[index1, ]
  #
  # index2 = seq(from = 2, to = n*p, by = p)
  # beta_hat2 = beta_hat[index2, ]
  #
  # matplot(lambda_list, t(beta_hat1), type = 'l', main = 'WOE',
  #         xlab = expression(symbol(l)), ylab = expression(symbol(b)[1]))
  # matplot(lambda_list, t(beta_hat2), type = 'l', main = 'WOE',
  #         xlab = expression(symbol(l)), ylab = expression(symbol(b)[2]))
  ...
  # K_star = K_hat[, index]
  # K_est_without_error = append(K_est_without_error, K_star)
  ...
  # RI_without_error = append(RI_without_error, RI)
  ...
  #   alpha1_without_error = if(alpha[1, 1] > 1) {
  #     rbind(alpha1_est_without_error, alpha[, 1])
  #   } else {
  #     rbind(alpha1_est_without_error, alpha[, 2])
  #   }
  #   alpha2_without_error = if(alpha[1, 2] < 1) {
  #     rbind(alpha2_without_error, alpha[, 2])
  #   } else {
  #     rbind(alpha2_without_error, alpha[, 1])
  #   }
  # 
  #   eta_without_error = rbind(eta_without_error, eta_star)
  # }
  # Iteration_sgaeiv(lambda_list[10], W1, W2, Z1, Z2, y, beta_initial_true, eta_initial_true, 3, 1, 2)
  ## --------------------SCAD --------------------------------------------------
  beta_hat = matrix(nrow = n * p, ncol = k)
  eta_hat = matrix(nrow = q, ncol = k)
  delta_hat = matrix(nrow = choose(n, 2) * p, ncol = k)
  ...
  # matplot(lambda_list, t(beta_hat2), type = 'l', main = 'MCP', 
  #         xlab = expression(symbol(l)), ylab = expression(symbol(b)[2]))
  plot(lambda_list, BIC, type = 'l', main = 'BIC')
  ...
  if(K_star == 2) {
    ...
    alpha = sapply(group_index, MeanBeta, beta_mat = beta_mat)
    alpha1_est_scad = append(alpha1_est_scad, ifelse(alpha[1] > 0, alpha[1], alpha[2]))
    alpha2_est_scad = append(alpha2_est_scad, ifelse(alpha[2] < 0, alpha[2], alpha[1]))
  ...
  if(K_star == 2) {
    ...
    alpha = sapply(group_index, MeanBeta, beta_mat = beta_mat)
    alpha1_est_mcp = append(alpha1_est_mcp, ifelse(alpha[1] > 0, alpha[1], alpha[2]))
    alpha2_est_mcp = append(alpha2_est_mcp, ifelse(alpha[2] < 0, alpha[2], alpha[1]))
  ...
  # beta_hat2 = beta_hat[index2, ]
  # 
  # matplot(lambda_list, t(beta_hat1), type = 'l', main = 'Ma', 
  #         xlab = expression(symbol(l)), ylab = expression(symbol(b)[1]))
  # matplot(lambda_list, t(beta_hat2), type = 'l', main = 'Ma', 
  #         xlab = expression(symbol(l)), ylab = expression(symbol(b)[2]))
  # plot(lambda_list, BIC, type = 'l', main = 'BIC')
  ...
  if(K_star == 2) {
    ...
    alpha = sapply(group_index, MeanBeta, beta_mat = beta_mat)
    alpha1_est_ma = append(alpha1_est_ma, ifelse(alpha[1] > 0, alpha[1], alpha[2]))
    alpha2_est_ma = append(alpha2_est_ma, ifelse(alpha[2] < 0, alpha[2], alpha[1]))
